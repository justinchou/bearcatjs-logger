		...
		pLogger[item] = function() {
			...
			if (!process.env.RAW_MESSAGE) {
				...
				if (args.length > 0) {
					...
					if (process.env.LOGGER_PROCESS) {
						prefixMessage = concatMessage(prefixMessage, process.pid);
					}
				}

				prefixMessage = colorize(prefixMessage, colours[item]);
			}

			if (args.length) {
				arguments[0] = prefixMessage + arguments[0];
			}
...
function initConfiguration(config) {
	...
	if (config.replaceConsole) {
		...
		console.log = console.originalLog.bind(console);
	}
}
